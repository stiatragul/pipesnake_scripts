...
function correction(c, output, k, X, MASK, pvalue, qvalue, threshold)
	...
	w = [[sort(arr[i:i+k-1])[P] for i in 1:length(arr)-k+1] for arr in w1]
...
function correction_multi(args, fin, fout)
	...
	temp = [string(arr[2:end]...) for arr in temp]
	temp = [split(arr, " ") for arr in temp]
	c = [string(arr...) for arr in temp]
	arrc = [Array{Char, 1}(str) for str in c]
	n = length(c[1])
	m = length(c)
	MASK = args["mask"]
