# Finds duplicate and problematic rows in lineage column of sample info file.
# USAGE: python script.py /path/to/your/csv_file.csv column_name
# Example: python BPA_SampleInfo.csv lineage
...
def find_issues_in_column(csv_file, column_name):
    ...
    issue_rows = df[df[column_name].str.contains(r'[()\\\/ ]', na=False)] # modify as needed
    ...
    if issue_rows.empty:
        print(f"No issues in column '{column_name}'.")
    else:
        print(f"Issues in column '{column_name}', possibly spaces, slashes, parentheses:")
        print(issue_rows)
